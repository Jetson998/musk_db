...
文件:   output/landing/index.html  （单文件，76,294 字节，零依赖）
...
3. 页面会产生的全部跳转（已核对源码）
========================================
A. 出站跳转 —— 全部指向 NewAPI 网关 https://db.muskapis.com，全部 target="_top"（共 9 处 + 顶栏 2 处 JS 重写）:
...
C. JS 行为（非跳转但涉及跨窗口）:
   ...
   - JS 常量 NEWAPI_BASE = "https://db.muskapis.com"（第 422 行）。
